...
export const MongoPaginationParamDecorator: () => ParameterDecorator = createParamDecorator(
  (
    ...
    const page: number = !isNaN(req.query[pageName]) ? Number(req.query[pageName]) : FIRST_PAGE;
    const limit: number = !isNaN(req.query[perPageName]) ? Number(req.query[perPageName]) : DEFAULT_NUMBER_OF_RESULTS;
    let filter: {};
    let sort: [];
